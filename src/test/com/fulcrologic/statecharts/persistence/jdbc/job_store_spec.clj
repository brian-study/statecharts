...
(deftest ^:integration create-job-persists-fields-test
  (testing "create-job! persists all fields correctly"
    (let [params (make-job-params {:payload {:key "value" :nested {:a 1}}
      ...
      (is (= "\"http\"" (:job-type row))
          "job-type is stored as pr-str'd form (self-describing shape)")
      (is (= "http" (sut/str->job-type (:job-type row)))
          "stored form round-trips to the original value")
