...
- Loss of natural poll-interval backoff: in the batch model, the coordinator naturally paused between batches. In the demand-driven model, when jobs complete rapidly, the coordinator can spin through claim-poll-submit cycles without pause. The `poll-interval-ms` only applies when no permits are available or no jobs are found, so a sustained burst of fast jobs (completing faster than one poll interval) keeps the coordinator in a tight claim loop with one `SELECT ... FOR UPDATE SKIP LOCKED` per iteration. With concurrency=5 and sub-ms jobs, this could mean hundreds of claim queries/second from a single worker. This is correct behavior (filling slots as fast as possible) but worth monitoring in production. If DB load becomes a concern, callers can increase `poll-interval-ms` or reduce `claim-limit`
